...
local data = {
	{ input = "#F00", output = { hex = "#FF0000", rgb = "rgb(255, 0, 0)", hsl = "hsl(0, 100%, 50%)" } },
	{
		input = "#F009",
		output = { hex = "#FF000099", rgb = "rgba(255, 0, 0 / 0.60)", hsl = "hsla(0, 100%, 50% / 0.60)" },
	},
	{ input = "#FF0000", output = { hex = "#FF0000", rgb = "rgb(255, 0, 0)", hsl = "hsl(0, 100%, 50%)" } },
	{
		input = "#FF000099",
		output = { hex = "#FF000099", rgb = "rgba(255, 0, 0 / 0.60)", hsl = "hsla(0, 100%, 50% / 0.60)" },
	},
	{ input = "rgb(255, 0, 0)", output = { hex = "#FF0000", rgb = "rgb(255, 0, 0)", hsl = "hsl(0, 100%, 50%)" } },
	{
		input = "rgba(255, 0, 0, 0.6)",
		output = { hex = "#FF000099", rgb = "rgba(255, 0, 0 / 0.60)", hsl = "hsla(0, 100%, 50% / 0.60)" },
	},
	{ input = "hsl(0, 100, 50%)", output = { hex = "#FF0000", rgb = "rgb(255, 0, 0)", hsl = "hsl(0, 100%, 50%)" } },
	{
...
for _, item in ipairs(data) do
	...
	if hexed ~= item.output.hex then
		print("for " .. item.input .. " expected " .. item.output.hex .. " got " .. hexed)
	end
	if rgbed ~= item.output.rgb then
		print("for " .. item.input .. " expected " .. item.output.rgb .. " got " .. rgbed)
	end
	if hsled ~= item.output.hsl then
		print("for " .. item.input .. " expected " .. item.output.hsl .. " got " .. hsled)
	end
end
